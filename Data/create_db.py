...
def create_formations_details_table():
    ...
    formations_id = [i[0] for i in cur.execute("Select id_Bloc FROM Formations").fetchall()]
...
if __name__ == "__main__":
    ...
    res = cur.execute("SELECT * FROM sqlite_master WHERE type='table' AND name=?", ("Formations",))

    print("Création de la table Formations")
    create_formations_table()
    # res = cur.execute("SELECT * FROM sqlite_master WHERE type='table' AND name=?", ("Formations_details",))
    # if not res.fetchone():
    print("Création de la table Formations_details")
    create_formations_details_table()
